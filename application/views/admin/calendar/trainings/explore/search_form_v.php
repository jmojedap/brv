...
<form accept-charset="utf-8" method="POST" id="search_form" @submit.prevent="get_list">
    ...
    <div id="adv_filters" style="<?= $filters_style ?>" class="mb-2">
        <div class="form-group row">
            <div class="col-md-9">
                <select name="cat_1" v-model="filters.cat_1" class="form-control" title="Filtrar por zona">
                    <option v-for="(option_room, key_room) in options_room" v-bind:value="key_room">{{ option_room }}</option>
                </select>
            </div>
            <label for="type" class="col-md-3 col-form-label">Zona</label>
        </div>
        ...
        <div class="form-group row">
            ...
            <label for="d1" class="col-md-3 col-form-label">Fecha</label>
        </div>
